...
class MultiRobotEnv:
    def __init__(self,launchfile, num_robots=2, num_goals=4):
        ...
        self.goals = np.random.randint(0, 4, size=(num_goals, 2))
        self.start_time = 0
        self.last_linear_velocities = np.zeros((num_robots,))
        self.last_angular_velocities = np.zeros((num_robots,))

        self.o_t_e = np.zeros((3, num_goals, 2), dtype=np.float32)
        self.o_t_o = np.zeros((num_robots, 3, num_goals, 2), dtype=np.float32)
        self.o_t_th = np.zeros((3), dtype=np.float32)
        self.laser_scanner = np.zeros((3, 360), dtype=np.float32)
    ...
    def step(self, actions):
        ...
        # Updating Last Linear Velocities
        for i in range(self.num_robots):
            self.last_linear_velocities = actions[i][0]
            self.last_angular_velocities = actions[i][1]
    ...
    def reset(self):
        ...
        # self.last_linear_velocities = 0
        # self.last_angular_velocities = 0
    ...
    def calculate_observation(self, robot):
        ...
        self.o_t_o = np.roll(self.o_t_o, shift=1, axis=1)
        for j in range(self.num_robots):
            if robot == j:
                continue
            for i, goal in enumerate(self.goals):
                ...
                self.o_t_o[j, 0, i, 1] = np.arctan2(dy, dx) - self.robot_orientations[j][2]   # Correction
        ...
        self.o_t_th = np.array((self.last_linear_velocities, self.last_angular_velocities))
        robot_observation.append(self.o_t_th)
